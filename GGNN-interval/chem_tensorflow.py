...
class ChemModel(object):
    ...
    def processAccInfo(self, pre, new, num_graphs):
        if pre == None:
            pre = [0,0,0,0]
        return [x + y for x, y in zip(pre, new)]
    ...
    def parseAccInfo(self, best_accInfo):
        precision = float(best_accInfo[0]) / (best_accInfo[0] + best_accInfo[2])
        recall = float(best_accInfo[0]) / (best_accInfo[0] + best_accInfo[3])
